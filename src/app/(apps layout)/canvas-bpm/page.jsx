...
const CanvasBPM = () => {
    ...
    useEffect(() => {
        const fetchProjects = async () => {
            ...
            setLoading(true);
            try {
                const res = await fetch(`/api/projects?tenantId=${activeTenantId}&category=BPM`);
                if (res.ok) {
                    const data = await res.json();
                    setProjects(data);
                }
            } catch (err) {
                console.error('Fetch BPM projects error:', err);
            } finally {
                setLoading(false);
            }
        };
        ...
        divKeys.forEach((divName, divIndex) => {
            ...
            generatedNodes.push({
                ...
                style: {
                    ...
                    border: `2px solid ${isDark ? '#3b82f6' : '#0d6efd'}`,
                    borderRadius: '12px',
                    padding: '10px 18px',
                    fontWeight: 700,
                    fontSize: '14px',
                    color: isDark ? '#93c5fd' : '#0d6efd',
                    minWidth: '180px',
                    textAlign: 'center',
                },
                draggable: true,
    ...
    const onNodeDoubleClick = useCallback((_, node) => {
        // Navigate to canvas if it's a project node
        ...
        return (
            <Container fluid="xxl">
                <div className="hk-pg-header pt-7 pb-4">
                    <h1 className="pg-title">📊 Kanvas BPM</h1>
                </div>
        ...
        <Container fluid="xxl">
            <div className="hk-pg-header pt-7 pb-4">
                <div className="d-flex justify-content-between align-items-center">
                    <div>
                        <h1 className="pg-title">📊 Kanvas BPM</h1>
                        <p className="text-muted mb-0">Bisnis Proses Manajemen — Tampilan kanvas seluruh proses BPM</p>
                    </div>
                    <div className="d-flex align-items-center gap-2">
                        <Badge bg="primary" className="px-3 py-2">{projects.length} Proyek BPM</Badge>
            ...
            <div style={{ width: '100%', height: 'calc(100vh - 200px)', borderRadius: '12px', overflow: 'hidden', border: `1px solid ${isDark ? '#334155' : '#e2e8f0'}` }}>
                ...
                >
                    <Controls />
                    <Background variant={BackgroundVariant.Dots} gap={20} size={1} color={isDark ? '#334155' : '#ddd'} />
